...
/**
 * Android-specific: create a device handle from a system file descriptor,
 * without requiring device enumeration via libusb_get_device_list().
 * Equivalent to libusb_wrap_sys_device() added in libusb 1.0.23.
 */
	...
	libusb_device_handle **dev_handle)
{
	struct libusb_device *dev;
	struct libusb_device_handle *_handle;
	size_t dev_priv_size = usbi_backend->device_priv_size;
	size_t handle_priv_size = usbi_backend->device_handle_priv_size;
	int r;

	*dev_handle = NULL;

	/* Allocate a minimal device object. priv data is zeroed (calloc),
	 * which is safe for op_destroy_device (it free()s NULL pointers). */
	dev = calloc(1, sizeof(*dev) + dev_priv_size);
	if (!dev)
		return LIBUSB_ERROR_NO_MEM;

	dev->ctx = ctx;
	dev->refcnt = 1;
	usbi_mutex_init(&dev->lock, NULL);

	_handle = calloc(1, sizeof(*_handle) + handle_priv_size);
	if (!_handle) {
		usbi_mutex_destroy(&dev->lock);
		free(dev);
		return LIBUSB_ERROR_NO_MEM;
	...
	/* Assign without calling libusb_ref_device so refcnt stays at 1.
	 * libusb_close -> libusb_unref_device will decrement to 0 and free dev. */
	_handle->dev = dev;
	_handle->claimed_interfaces = 0;

	r = usbi_backend->open2(_handle, (int)sys_dev);
	if (r < 0) {
		usbi_mutex_destroy(&dev->lock);
		free(dev);
		usbi_mutex_destroy(&_handle->lock);
