...
/* Synchronous USBDEVFS_BULK loop for Android.
 * Samsung (and other Android vendors) route USB through their USB service;
 * USBDEVFS_SUBMITURB (async URB) is ownership-checked and fails with ENODEV,
 * but USBDEVFS_BULK (synchronous, used by UsbDeviceConnection.bulkTransfer())
 * is not and works correctly. */
static int mirisdr_read_sync_android(mirisdr_dev_t *p, mirisdr_read_async_cb_t cb, void *ctx, uint32_t num, uint32_t len)
{
    int fd = libusb_get_fd(p->dh);
    if (fd < 0) return -1;

    /* Set alternate setting for BULK (interface 0, alt 3) */
    struct usbdevfs_setinterface si = { .interface = 0, .altsetting = 3 };
    if (ioctl(fd, USBDEVFS_SETINTERFACE, &si) < 0) {
        MLOG("USBDEVFS_SETINTERFACE alt=3 failed: %s (trying alt=0)", strerror(errno));
        si.altsetting = 0;
        ioctl(fd, USBDEVFS_SETINTERFACE, &si);
    }

    /* Clear halt on endpoint 0x81 */
    unsigned int ep = 0x81;
    ioctl(fd, USBDEVFS_CLEAR_HALT, &ep);

    mirisdr_streaming_start(p);
    p->async_status = MIRISDR_ASYNC_RUNNING;

    unsigned char *buf = malloc(DEFAULT_BULK_BUFFER);
    if (!buf) return -1;

    while (p->async_status == MIRISDR_ASYNC_RUNNING) {
        struct usbdevfs_bulktransfer bulk = {
            .ep      = 0x81,
            .len     = DEFAULT_BULK_BUFFER,
            .timeout = DEFAULT_BULK_TIMEOUT,
            .data    = buf,
        };
        int r = ioctl(fd, USBDEVFS_BULK, &bulk);
        if (r < 0) {
            if (errno == EINTR) continue;
            MLOG("USBDEVFS_BULK error: %s (errno=%d)", strerror(errno), errno);
            break;
        }
        if (r > 0) cb((unsigned char *)buf, r, ctx);
    }
...
#ifdef __ANDROID__
    /* On Android, USBDEVFS_SUBMITURB is blocked when the interface was claimed
     * by the Android USB service (not by our process's fd). Use the synchronous
     * USBDEVFS_BULK ioctl instead, which is what UsbDeviceConnection uses. */
    return mirisdr_read_sync_android(p, cb, ctx, num, len);
#endif
...
retry_transfer:
    ...
    for (i = 0; i < p->xfer_buf_num; i++) {
        ...
        if (r < 0) {
            ...
            /* BULK: continue with successfully submitted transfers */
            if (i >= 2) {
                size_t k;
                fprintf(stderr, "libmirisdr: continuing with %zu BULK transfers\n", i);
                for (k = i; k < p->xfer_buf_num; k++) {
                    if (p->xfer[k]) {
                        libusb_free_transfer(p->xfer[k]);
                        p->xfer[k] = NULL;
                    }
                    if (p->xfer_buf[k]) {
                        free(p->xfer_buf[k]);
...
failed_free:
    ...
    if (p->xfer) {
        ...
        {
            struct timeval drain_tv = {0, 200000};
            int drain_tries = 0;
            while (drain_tries < 10) {
                int done = 1;
                for (j = 0; j < p->xfer_buf_num; j++) {
